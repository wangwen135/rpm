...
public class CommandHandler extends ChannelInboundHandlerAdapter {
    ...
    private void forwardResultHandler(ChannelHandlerContext ctx, ForwardResultPacket forwardResult) {
        ...
        // 转发指令成功
        ctx.write(new SuccessPacket(new Random().nextInt()));
    ...
    private void forwardCommandHandler(ChannelHandlerContext ctx, ForwardCommandPacket forwardCommand) {
        ...
        EventLoopGroup workerGroup = masterServer.getServerManager().getWorkerGroup();
        b.group(workerGroup).channel(inboundChannel.getClass());
        ...
        f.addListener(new ChannelFutureListener() {
            ...
            public void operationComplete(ChannelFuture future) throws Exception {
                if (future.isSuccess()) {
                    ...
                    inboundChannel.writeAndFlush(new SuccessPacket(new Random().nextInt()));
                    ...
                    logger.warn("到目标地址{}：{} 的连接建失败，关闭客户端链路", forwardCommand.getHost(), forwardCommand.getPort());
                    inboundChannel.writeAndFlush(new FailPacket(
                            "到目标地址" + forwardCommand.getHost() + "：" + forwardCommand.getPort() + " 的连接建失败"));
